...
<g class='axis x-axis'>
	{#each tickVals as tick, i}
		<g class='tick tick-{ tick }' transform='translate({$xScale(tick)},{$yRange[0]})'>
			{#if gridlines !== false}
				<line y1='{($height * -1)}' y2='{(median && Array.isArray(ticks) && i == Math.floor(ticks.length / 2)) || (stagger && i % 2 == 0) ? dyTick + yTick : dyTick + yTick - 13}' x1='0' x2='0' stroke='{lineColor}' stroke-width="{median && Array.isArray(ticks) && i == Math.floor(ticks.length / 2) ? 2 : 1}" class:baseline={solid}></line>
			{/if}
			<text
				x="{xTick || isBandwidth ? $xScale.bandwidth() / 2 : 0 }"
				y='{flipped ? ($height * -1) - 6 : (median && Array.isArray(ticks) && i == Math.floor(ticks.length / 2)) || (stagger && i % 2 == 0) ? yTick + 13 : yTick}'
